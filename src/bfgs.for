C     Last change:  CA   25 Sep 2015    2:59 pm
      SUBROUTINE BFGS(NPAR,XVALS)
C
C BFGS 
C
      IMPLICIT NONE
...
c     the driver.
      ...
      LOGICAL lsave(4)
      INTEGER m,iprint,nbd(nmax),iwa(3*nmax),isave(44),I,J
      REAL(8) f,factr,pgtol, l(nmax),u(nmax),g(nmax),
     +       dsave(29),wa(lenwa),YVARS(100),SS
      REAL(8) HITFUN2
      EXTERNAL HITFUN2
...
c     This is the call to the L-BFGS-B code.
      ...
      call setulb(NPAR,m,XVALS,l,u,nbd,f,g,factr,pgtol,wa,iwa,
     +            task,iprint,csave,lsave,isave,dsave)
 
      IF (task(1:2) .eq. 'FG') THEN

         f = HITFUN2(XVALS,NPAR)
         
         DO 22 I = 1,NPAR
          DO 221 J = 1,NPAR
           YVARS(J) = XVALS(J)
221       CONTINUE           
          YVARS(I) = (1.0e-20+XVALS(I))*1.01
          SS = HITFUN2(YVARS,NPAR)
          g(I) = (SS-f)/((1.0e-20+XVALS(I))*0.01)
C          WRITE(*,600) I,XVALS(I),f,SS,g(I)
22       CONTINUE          
